...
const tabs: { key: TabKey; label: string }[] = [
  { key: 'APPOINTMENT', label: '挂号通知' },
  { key: 'SYSTEM', label: '系统通知' },
]
type TabKey = 'APPOINTMENT' | 'SYSTEM'

const activeTab = ref<TabKey>('APPOINTMENT')

// 每个标签页独立的列表和分页状态
const dataMap = ref<Record<TabKey, { list: Notification[]; total: number; page: number; noMore: boolean }>>({
  APPOINTMENT: { list: [], total: 0, page: 1, noMore: false },
...
// 启动时预加载所有标签页的第一页
onMounted(async () => {
  await fetchTabData('APPOINTMENT', 1, false)
