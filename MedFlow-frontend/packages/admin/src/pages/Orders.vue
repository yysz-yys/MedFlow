<template>
  <div class="page">
    ...
    <el-table :data="sortedList" border stripe v-loading="loading" @sort-change="handleSortChange">
      ...
      <el-table-column prop="patient_name" label="患者" min-width="80" sortable="custom" />
...
async function handleCancel(row: DrugOrder) {
  try {
    await ElMessageBox.confirm('确定要取消该订单吗？库存将退还。')
...
async function handleComplete(row: DrugOrder) {
  try {
    await ElMessageBox.confirm('确定标记为已取药吗？')
...
async function handleUncomplete(row: DrugOrder) {
  try {
    await ElMessageBox.confirm('确定要撤回取药确认吗？')
...
async function handleRestore(row: DrugOrder) {
  try {
    await ElMessageBox.confirm('确定要恢复该订单吗？将重新扣减库存。')
